...
def main():
    ...
    print ('')
    print ('--- ITEM PAGE ---')
    print ('ITEM TITLE: {}'.format(soup.find(id='productTitle').text.replace(' ', '').replace('\n', '')))
    #print ('phantomjs driver open url: {}'.format(driver.current_url))
    ...
    print ('STEALTH PERCENTAGE: {}%'.format(int(100*stealth_df['stealth_probability'].sum()/df.shape[0])))
    print ('HI PROBABILITY STEALTH REVIEWER COUNT: {}'.format(high_stealth_df.shape[0]))
    print ('LOW PROBABILITY STEALTH REVIEWER COUNT: {}'.format(low_stealth_df.shape[0]))
    print ('WRONG DATA REVIEWER COUNT : {}'.format(wrong_df.shape[0]))
